...
///////////////////////////////////////////////////////////////////////// 
/* Crear el nuevo predictor de salto */
  case BPredGskew:
    pred->dirpred.gskew = 
      bpred_dir_create(class, l1size, l2size, shift_width, xor);
...
/////////////////////////////////////////////////////////////////////////
  ...
  case BPredGskew:
	...
	fatal("level-1 size, `%d', must be non-zero and a power of two", 
	      l1size);
      pred_dir->config.gskew.l1size = l1size;
      
      if (!l2size || (l2size & (l2size-1)) != 0)
	fatal("level-2 size, `%d', must be non-zero and a power of two", 
	      l2size);
      pred_dir->config.gskew.l2size = l2size;
      
      if (!shift_width || shift_width > 30)
	fatal("shift register width, `%d', must be non-zero and positive",
	      shift_width);
      pred_dir->config.gskew.shift_width = shift_width;
      
      pred_dir->config.gskew.xor = xor;
      pred_dir->config.gskew.shiftregs = calloc(l1size, sizeof(int));
      if (!pred_dir->config.gskew.shiftregs)
	fatal("cannot allocate shift register table");
      
      pred_dir->config.gskew.l2table = calloc(l2size, sizeof(unsigned char)); /* Reservar memoria tabla 1 */
      if (!pred_dir->config.gskew.l2table)
	fatal("cannot allocate second level table");
	
      pred_dir->config.gskew.l2table2 = calloc(l2size, sizeof(unsigned char)); /* Reservar memoria tabla 2 */
      if (!pred_dir->config.gskew.l2table2)
	fatal("cannot allocate second level table");
	
      pred_dir->config.gskew.l2table3 = calloc(l2size, sizeof(unsigned char)); /* Reservar memoria tabla 3 */
      if (!pred_dir->config.gskew.l2table3)
	fatal("cannot allocate second level table");

      /* initialize counters to weakly this-or-that */
      flipflop = 3;
      for (cnt = 0; cnt < l2size; cnt++) /* inicializar las 3 tablas para tener StronglyTaken ->(11) */
	{
	  pred_dir->config.gskew.l2table[cnt] = flipflop;
	  //flipflop = 3 - flipflop;
	  pred_dir->config.gskew.l2table2[cnt] = flipflop;
	  pred_dir->config.gskew.l2table3[cnt] = flipflop;
	}
      break;
    }
/////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////
  case BPredGskew:
    fprintf(stream,
      ...
      name, pred_dir->config.gskew.l1size, pred_dir->config.gskew.shift_width,
      pred_dir->config.gskew.xor ? "" : "no", pred_dir->config.gskew.l2size);
    break;
/////////////////////////////////////////////////////////////////////////
    ...
    case BPredGskew:
      {
        ...
        /* traverse 2-level tables */
        l1index = (baddr >> MD_BR_SHIFT) & (pred_dir->config.gskew.l1size - 1);
        l2index = pred_dir->config.gskew.shiftregs[l1index];
        
        if (pred_dir->config.gskew.xor == 1) {
             l2index =  ~(l2index ^ (baddr >> MD_BR_SHIFT)); /* XNOR */
        }
        
        if (pred_dir->config.gskew.xor == 2){
             l2index = l2index | ((baddr >> MD_BR_SHIFT) << pred_dir->config.gskew.shift_width); /* concatenar */
        }  
        
        if (pred_dir->config.gskew.xor == 3){
             int valor = log2(pred_dir->config.gskew.l2size) - pred_dir->config.gskew.shift_width; /* concatenar */
             if (valor<1){valor = 1;}
             l2index =  (baddr >> MD_BR_SHIFT) | (l2index << valor );
        }    
        
        l2index = l2index & (pred_dir->config.gskew.l2size - 1);
        
        p = &pred_dir->config.gskew.l2table[l2index];           
      }
      break;
/////////////////////////////////////////////////////////////////////////      
    case BPred2bit:
...
/////////////////////////////////////////////////////////////////////////
/* Asignar valor al XOR */
    case BPredGskew:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	 	pred->dirpred.gskew->config.gskew.xor = 1;
	  	dir_update_ptr->pdir1 = bpred_dir_lookup (pred->dirpred.gskew, baddr);
	  
		pred->dirpred.gskew->config.gskew.xor = 2;
	  	dir_update_ptr->pdir2 = bpred_dir_lookup (pred->dirpred.gskew, baddr);
	  
	  	pred->dirpred.gskew->config.gskew.xor = 3;
	  	dir_update_ptr->pdir3 = bpred_dir_lookup (pred->dirpred.gskew, baddr);
...
/////////////////////////////////////////////////////////////////////////
if (pred->class == BPredGskew){
  if (pbtb == NULL)
    {     
      return (( (*(dir_update_ptr->pdir1)>>1) + (*(dir_update_ptr->pdir2)>>1) + (*(dir_update_ptr->pdir3)>>1) >= 2) ? 1: 0);
    }
  else
    {
      return (( (*(dir_update_ptr->pdir1)>>1) + (*(dir_update_ptr->pdir2)>>1) + (*(dir_update_ptr->pdir3)>>1) >= 2) ? pbtb->target : 0);  
    }
}
/////////////////////////////////////////////////////////////////////////
...
/* Actualizar GBHR y PHT */
  
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) && (pred->class == BPredGskew))
    {
      int l1index, shift_reg;

      l1index =(baddr >> MD_BR_SHIFT) & (pred->dirpred.gskew->config.gskew.l1size - 1);
      shift_reg = (pred->dirpred.gskew->config.gskew.shiftregs[l1index] << 1) | (!!taken);
      pred->dirpred.gskew->config.gskew.shiftregs[l1index] = shift_reg & ((1 << pred->dirpred.gskew->config.gskew.shift_width) - 1);
		
	if (!!taken != !!pred_taken){

		if (!!taken){
			 *dir_update_ptr->pdir1 = *dir_update_ptr->pdir1+1;
		 	if (*dir_update_ptr->pdir1>3) *dir_update_ptr->pdir1 = 3;
			 *dir_update_ptr->pdir2 = *dir_update_ptr->pdir2+1;
 			if (*dir_update_ptr->pdir2>3) *dir_update_ptr->pdir2 = 3;
			 *dir_update_ptr->pdir3 = *dir_update_ptr->pdir3+1;
 			if (*dir_update_ptr->pdir3>3) *dir_update_ptr->pdir3 = 3;
			}
		else{
			 *dir_update_ptr->pdir1 = *dir_update_ptr->pdir1-1;
 			if (*dir_update_ptr->pdir1<0) *dir_update_ptr->pdir1 = 0;
			 *dir_update_ptr->pdir2 = *dir_update_ptr->pdir2-1;
 			if (*dir_update_ptr->pdir2<0) *dir_update_ptr->pdir2 = 0;
			 *dir_update_ptr->pdir3 = *dir_update_ptr->pdir3-1;
 			if (*dir_update_ptr->pdir3<0) *dir_update_ptr->pdir3 = 0;
		}
	}	
}
/////////////////////////////////////////////////////////////////////////
...
if (dir_update_ptr->pdir3)
    {
      if (taken)
	{
	  if (*dir_update_ptr->pdir3 < 3)
	    ++*dir_update_ptr->pdir3;
	}
      else
	{ /* not taken */
	  if (*dir_update_ptr->pdir3 > 0)
	    --*dir_update_ptr->pdir3;
	}
    }
/////////////////////////////////////////////////////////////////////////
